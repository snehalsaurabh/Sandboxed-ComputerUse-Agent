...
export type BrowserToolAction =
  ...
  | {
      tool: "browser_type";
      input: { selector: string; text: string; clear?: boolean; pressEnter?: boolean };
    }
  | {
...
export class BrowserToolFamily {
  ...
  /**
   * Models sometimes skip browser_open and start with browser_goto. Open a session once
   * before any tool that needs a page.
   */
  private async ensureSessionOpen(): Promise<void> {
    const c = this.getController();
    if (c.isOpen()) {
      return;
    }
    await c.open();
    this.emit("browser_session_started", {
  ...
  async execute(action: BrowserToolAction): Promise<ToolResult> {
    ...
    switch (action.tool) {
      ...
      case "browser_type": {
        ...
        await locator.pressSequentially(action.input.text);
        if (action.input.pressEnter) {
